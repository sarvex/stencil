...
/**
 * Helper function to determine which version string to use. Primarily used to ensure testing coverage of picking the
 * correct version number is provided.
...
export async function promptRelease(opts: BuildOptions): Promise<NormalizedReleaseResponses> {
  ...
  try {
    ...
    return {
      confirm: answers.confirm,
      otp: answers.otp,
      tagToUse: determineAnsweredTagToUse(answers),
    };
  } catch (err: any) {
    console.log('\n', color.red(err), '\n');
    process.exit(0);
...
/**
 * Helper function to determine which tag string to use. Primarily used to ensure testing coverage of picking the
 * correct tag string is provided.
 * @param answers user provided answers to pick from
